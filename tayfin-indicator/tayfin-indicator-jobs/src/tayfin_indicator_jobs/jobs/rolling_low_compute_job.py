"""RollingLowComputeJob — compute rolling low and store in indicator_series.

For each ticker in the configured index:
1. Fetch OHLCV via ingestor API
2. Compute rolling min of close over configured window (252 = 52-week)
...
from ..indicator.compute import compute_rolling_low
...
class RollingLowComputeJob:
    """Compute rolling low indicators for a given target index."""
    ...
    @classmethod
    def from_config(cls, target_name: str, target_cfg: dict, full_cfg: dict) -> "RollingLowComputeJob":
    ...
    def run(self, ticker: str | None = None, limit: int | None = None) -> None:
        ...
        # Extract rolling_low windows from config
        rl_windows: list[int] = []
        for ind in indicators:
            if ind.get("key") == "rolling_low":
                w = ind.get("params", {}).get("window")
                if w:
                    rl_windows.append(int(w))
        ...
        job_run_id = self.job_run_repo.create(
            job_name="rolling_low_compute",
            target_name=self.target_name,
            status="RUNNING",
            params=params,
        )

        typer.echo(f"[rolling_low_compute] job_run_id  = {job_run_id}")
        typer.echo(f"[rolling_low_compute] target      = {self.target_name}")
        typer.echo(f"[rolling_low_compute] index_code  = {index_code}")
        typer.echo(f"[rolling_low_compute] date range  = {start_date} → {end_date}")
        ...
        typer.echo(f"[rolling_low_compute] tickers     = {len(tickers)}")
        typer.echo(f"[rolling_low_compute] windows     = {rl_windows}")
        ...
        for tkr in tickers:
            result = self._process_ticker(
                tkr, rl_windows, start_date, end_date, job_run_id,
            )
        ...
        typer.echo(f"[rolling_low_compute] status = {final_status}")
    ...
    def _process_ticker(
        ...
        """Fetch OHLCV, compute rolling low, upsert rows. Returns summary dict."""
        result: dict = {
            "ticker": tkr,
            "status": "SUCCESS",
            "candles_fetched": 0,
