"""AtrComputeJob — compute ATR indicators and store in indicator_series.

For each ticker in the configured index:
1. Fetch OHLCV via ingestor API
2. Compute ATR (simple moving average of True Range)
...
class AtrComputeJob:
    """Compute ATR indicators for a given target index."""
    ...
    @classmethod
    def from_config(cls, target_name: str, target_cfg: dict, full_cfg: dict) -> "AtrComputeJob":
    ...
    def run(self, ticker: str | None = None, limit: int | None = None) -> None:
        ...
        # Extract ATR windows from config
        atr_windows: list[int] = []
        for ind in indicators:
            if ind.get("key") == "atr":
                w = ind.get("params", {}).get("window")
                if w:
                    atr_windows.append(int(w))
        ...
        job_run_id = self.job_run_repo.create(
            job_name="atr_compute",
            target_name=self.target_name,
            status="RUNNING",
            params=params,
        )

        typer.echo(f"[atr_compute] job_run_id  = {job_run_id}")
        typer.echo(f"[atr_compute] target      = {self.target_name}")
        typer.echo(f"[atr_compute] index_code  = {index_code}")
        typer.echo(f"[atr_compute] date range  = {start_date} → {end_date}")
        ...
        typer.echo(f"[atr_compute] tickers     = {len(tickers)}")
        typer.echo(f"[atr_compute] atr windows = {atr_windows}")
        ...
        for tkr in tickers:
            result = self._process_ticker(
                tkr, atr_windows, start_date, end_date, job_run_id,
            )
        ...
        typer.echo(f"[atr_compute] status = {final_status}")
    ...
    def _process_ticker(
        ...
        """Fetch OHLCV, compute ATR, upsert rows. Returns summary dict."""
        result: dict = {
            "ticker": tkr,
            "status": "SUCCESS",
            "candles_fetched": 0,
        ...
        try:
            ...
            # Need at least 2 rows (previous close required for TR)
            if len(df) < 2:
