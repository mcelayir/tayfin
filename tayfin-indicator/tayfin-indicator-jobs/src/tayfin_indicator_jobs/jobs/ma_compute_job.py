"""MaComputeJob — compute SMA indicators and store in indicator_series.

For each ticker in the configured index:
1. Fetch OHLCV via ingestor API
2. Compute SMA windows from config
...
class MaComputeJob:
    """Compute SMA indicators for a given target index."""
    ...
    def run(self, ticker: str | None = None, limit: int | None = None) -> None:
        ...
        typer.echo(f"[ma_compute] date range  = {start_date} → {end_date}")
        ...
        typer.echo(f"[ma_compute] tickers     = {len(tickers)}")
        ...
        for ind in indicators:
            if ind.get("key") == "sma":
                ...
                if w:
                    sma_windows.append(int(w))
        typer.echo(f"[ma_compute] sma windows = {sma_windows}")
        ...
        for tkr in tickers:
            result = self._process_ticker(
                tkr, sma_windows, start_date, end_date, job_run_id,
            )
        ...
        typer.echo(f"[ma_compute] status = {final_status}")
    ...
    def _process_ticker(
        ...
        """Fetch OHLCV, compute SMAs, upsert rows. Returns summary dict."""
        result: dict = {
            "ticker": tkr,
            "status": "SUCCESS",
            "candles_fetched": 0,
