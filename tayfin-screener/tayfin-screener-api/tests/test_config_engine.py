"""Unit tests for tayfin-screener-api config & engine standardization.

Tests cover ADR-04 (config loader), ADR-05 (env var contract), and
ADR-06 (default credential policy).  Also verifies the engine move
from db/__init__.py to db/engine.py with backward-compatible re-export.
"""
...
class TestConfigLoader:
    ...
    def test_load_config_returns_dict(self, tmp_path: Path):
        ...
        from tayfin_screener_api.config.loader import load_config
    ...
    def test_load_config_reads_yaml(self, tmp_path: Path):
        ...
        assert result == {"server": {"host": "0.0.0.0", "port": 8020}}
    ...
    def test_config_package_reexport(self):
        ...
        from tayfin_screener_api.config import load_config as from_init
        from tayfin_screener_api.config.loader import load_config as from_loader
...
class TestGetEngine:
    ...
    def setup_method(self):
        ...
        from tayfin_screener_api.db.engine import reset_engine
    ...
    def test_get_engine_returns_engine(self):
        ...
        from tayfin_screener_api.db.engine import get_engine
    ...
    def test_reset_engine_clears_singleton(self):
        ...
        from tayfin_screener_api.db.engine import get_engine, reset_engine
    ...
    def test_default_credentials_match_adr06(self):
        ...
        with mock.patch.dict(os.environ, env_patch, clear=True):
            ...
            # ADR-06: default password must be empty, NOT "tayfin_password"
            assert "tayfin_password" not in url, (
                f"Default password should be empty per ADR-06, got: {url}"
            )
...
# ================================================================
# C) Engine Re-export Tests (structural move)
# ================================================================


class TestEngineReexport:
    """Verify the engine moved from db/__init__.py to db/engine.py
    while preserving backward-compatible imports."""

    def test_import_from_db_package(self):
        """from tayfin_screener_api.db import get_engine works."""
        from tayfin_screener_api.db import get_engine

        assert callable(get_engine)

    def test_import_from_db_engine_module(self):
        """from tayfin_screener_api.db.engine import get_engine works."""
        from tayfin_screener_api.db.engine import get_engine
    ...
    def test_both_imports_are_same_function(self):
        ...
        from tayfin_screener_api.db import get_engine as from_pkg
        from tayfin_screener_api.db.engine import get_engine as from_mod

        assert from_pkg is from_mod

    def test_reset_engine_reexported(self):
        """reset_engine is also available via the db package."""
        from tayfin_screener_api.db import reset_engine
