"""Unit tests for tayfin-ingestor-api config & engine standardization.

Tests cover ADR-04 (config loader), ADR-05 (env var contract), and
ADR-06 (default credential policy).

These are pure unit tests — they mock the filesystem and env vars,
...
class TestConfigLoader:
    ...
    def test_load_config_returns_dict(self, tmp_path: Path):
        ...
        from tayfin_ingestor_api.config.loader import load_config
    ...
    def test_config_package_reexport(self):
        ...
        from tayfin_ingestor_api.config import load_config as from_init
        from tayfin_ingestor_api.config.loader import load_config as from_loader
...
class TestGetEngine:
    ...
    def setup_method(self):
        ...
        from tayfin_ingestor_api.db.engine import reset_engine
    ...
    def test_reset_engine_clears_singleton(self):
        ...
        from tayfin_ingestor_api.db.engine import get_engine, reset_engine
    ...
    def test_default_credentials_match_adr06(self):
        ...
        # Clear all POSTGRES_* env vars to test defaults
        env_patch = {
            k: v for k, v in os.environ.items()
            if not k.startswith("POSTGRES_") and not k.startswith("DB_")
        }
        with mock.patch.dict(os.environ, env_patch, clear=True):
            ...
            assert "localhost" in url, f"Expected localhost in URL, got: {url}"
            assert "5432" in url, f"Expected port 5432 in URL, got: {url}"
            assert "/tayfin" in url, f"Expected /tayfin in URL, got: {url}"
    ...
    def test_db_star_vars_no_longer_supported(self):
        """DB_* env vars must NOT be read (ADR-05 retired them)."""
        from tayfin_ingestor_api.db.engine import get_engine, reset_engine

        # Set DB_* vars but NOT POSTGRES_* — engine should use defaults
        env_patch = {
            k: v for k, v in os.environ.items()
            if not k.startswith("POSTGRES_") and not k.startswith("DB_")
        }
        env_patch.update({
            "DB_HOST": "should-not-appear",
            "DB_PORT": "6666",
            "DB_USER": "old_user",
            "DB_PASS": "old_pass",
            "DB_NAME": "old_db",
        ...
        with mock.patch.dict(os.environ, env_patch, clear=True):
            ...
            assert "should-not-appear" not in url, "DB_HOST should be ignored"
            assert "6666" not in url, "DB_PORT should be ignored"
            assert "old_user" not in url, "DB_USER should be ignored"
            assert "old_db" not in url, "DB_NAME should be ignored"
